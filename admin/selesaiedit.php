      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <div class="card card-primary">
            ...
            ?>
            <form method="POST" enctype="multipart/form-data" action="action/selesaieditact">
              <div class="card-body">
                <div class="form-group">
                  <label for="exampleInputEmail1">No Invoice</label>
                  <input type="text" class="form-control" name="no_invoice" id="exampleInputEmail1" value="<?php echo htmlentities($result['no_invoice']); ?>" placeholder="Enter email" readonly>
                </div>
                <div class="form-group">
                ...
                <div class="form-row">
                  <div class="form-group col-md-4">
                    <label for="inputCity">Bukti Transfer</label>
                    <?php
                    $bukti = $result['bukti'];
                    if ($bukti == "Offline") {
                      echo '<input type="text" class="form-control" id="inputCity" value="Pembelian Offline" disabled>';
                    } else {
                      echo "<a href='../bukti/$bukti' target='_blank'><img style='border: none;' src='../bukti/$bukti' width=' 300' height='200' style=' border:solid 1px #000' alt='../bukti/$bukti'></a>";
                    }
                    ?>
                  </div>
                </div>
                <div class="form-group">
                  <label for="inputCity">Resi Pengiriman</label>

                  <?php
                  $resi = $result['resi_pengiriman'];
                  if ($resi == "Pickup") {
                    echo "<input type='text' value='$resi' name='resi_pengiriman' class='form-control' placeholder='Masukkan nomer resi' disabled>";
                  } else {
                    echo "<input type='text' value='$resi' name='resi_pengiriman' class='form-control' placeholder='Masukkan nomer resi'>";
                  }
                  ?>
                ...
                <div class="card-footer">
                  ...
                  $resi = $result['resi_pengiriman'];
                  if ($resi == "Pickup") {
                    echo "<button type='submit' disabled class='btn btn-primary' onclick='return confirm(' Apakah anda yakin akan mengubah nomor resi pengiriman?');'>Ubah Resi Pengiriman</button>";
                  } else {
                    echo "<button type='submit' class='btn btn-primary' onclick='return confirm(' Apakah anda yakin akan mengubah nomor resi pengiriman?');'>Ubah Resi Pengiriman</button>";
                  }
                  ?>
                </div>
            </form>
