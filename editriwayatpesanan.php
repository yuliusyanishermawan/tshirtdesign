...
if (strlen($_SESSION['user']) == 0) {
    ...
    $title = "Edit Riwayat Pesanan";
    include('includes/config.php');
    include('includes/header.php');
?>

    <?php
    if (isset($_POST['kirimulasan'])) {
        $no_invoice = $_POST['no_invoice'];
        $ulasan = $_POST['ulasan'];
        $ratings = $_POST['ratings'];
        $sql     = "UPDATE tbl_order SET ulasan='$ulasan', ratings='$ratings' WHERE no_invoice='$no_invoice'";
        $query     = mysqli_query($koneksidb, $sql);
        if ($query) {
            echo "<script type='text/javascript'>
                  alert('Terimakasih telah memberikan rating kepada kami.'); 
                  document.location = 'riwayatpesanan.php'; 
              </script>";
        } else {
            echo "<script type='text/javascript'>
            ...
            <div class="row justify-content-center">
                <main class="col-md-9">
                    <div class="card card-body">
                        ...
                        <form class="needs-validation" action="action/editriwayatpesananact.php" method="POST" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="firstName" name="no_invoice" placeholder="Input nama desain..." value="<?php echo htmlentities($result['no_invoice']); ?>" required readonly>
                            </div>
                            <div class="form-group">
                            ...
                            if ($status != "Menunggu Pembayaran") {
                                ...
                                <div class="input-group">
                                    <div class="custom-file">
                                        <input type="file" id="input" accept="image/*" required name="bukti" onchange="changevalue(); changelabel1();" aria-describedby="inputGroupFileAddon01">
                                ...
                                <label for="inputCity">Bukti Transfer</label><br>
                                        <img id="buktiimg" style="max-width: 500px; max-height: 500px; border:none;" src="bukti/';
                                echo $bukti;
                                echo '" alt="">
                                     </div> 
                                    </div>';
                                ...
                                <div class="input-group">
                                    <div class="custom-file">
                                        <input type="file" id="input" accept="image/*" required name="bukti" onchange="changevalue(); changelabel1();" aria-describedby="inputGroupFileAddon01">
                                        <label class="custom-file-label" for="input" id="label">Choose file</label>
                                    </div>
                                </div>
                            </div>';
                                echo '<div class="form-row "><div class="form-group">
                                <label for="inputCity">Preview Bukti Transfer</label><br>
                                        <img id="buktiimg" style="max-width: 500px; max-height: 500px; border:none;" alt="">
                                     </div> 
                                    </div>';
                            };
                            ?>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <figure class="itemside">
                                                    <figcaption class="info">
                                                        <a class="title text-dark"><?php echo htmlentities($result['nama_order']); ?></a>
                                            ...
                                            <td>
                                                ...
                                                <div class="price-wrap">
                                                    <var class="price" id="harga"><?php echo "Rp" . number_format($result['total_harga_desain_produk_user'], 0, ",", "."); ?></var>
                                                </div> <!-- price-wrap .// -->
                                            </td>
                                        </tr>
                            ...
                            <button class="btn btn-primary btn-lg btn-block" type="submit">Upload Bukti</button>
                    </div> <!-- card.// -->
                </main> <!-- col.// -->
                    ...
                    <form method="POST">
                        <div class="card">
                            <div class="card-body">
                                ...
                                ?>
                                <div class="form-group">
                                    <label for="firstName">Ulasan</label></label>
                                    <input type="text" class="form-control" placeholder="Beri Ulasan" required name="ulasan">
                                    <input type="text" class="form-control" id="firstName" name="no_invoice" placeholder="Input nama desain..." value="<?php echo htmlentities($result['no_invoice']); ?>" required hidden>
                                </div>
                                <div class="form-group">
                                    <label for="exampleFormControlSelect1">Ratings</label>
                                    <select class="form-control" required name="ratings">
                                ...
                                <button class="btn btn-primary btn-sm btn-block" type="submit" name="kirimulasan">Kirim Ulasan</button>
                    </form>
            </div> <!-- card-body.// -->
        </div> <!-- card .// -->
    ...
    <script>
        function changelabel1() {
            document.getElementById("label").innerHTML = document.getElementById('input').files[0].name;
        };

        function changevalue() {
            document.getElementById("buktiimg").style.display = "block";
            var oFReader = new FileReader();
            oFReader.readAsDataURL(document.getElementById("input").files[0]);

            oFReader.onload = function(oFREvent) {
                document.getElementById("buktiimg").src = oFREvent.target.result;
            };
        }
    </script>
